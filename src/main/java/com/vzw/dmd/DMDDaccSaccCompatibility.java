...
public class DMDDaccSaccCompatibility extends HttpServlet{
	private static Logger L =
		Logger.getLogger(DMDLogs.getLogName(DMDDaccSaccCompatibility.class));
	/**
	...
	*/
	public void defaultAction(HttpServletRequest req, HttpServletResponse res)
		...
		DaccSaccCompatibilityVO compatibilityVO= new DaccSaccCompatibilityVO();		
		ServletOutputStream out = res.getOutputStream();
		boolean proceedToGetData = false;
		try{
			String dacc = req.getParameter(DMDConstants.COMPA_INPUT_DEVICE_DACC);
			if(dacc == null || dacc.trim().equals(DMDConstants.EMPTY_STRING)){
				compatibilityVO.setStatusCode(DMDConstants.STATUS_CODE__INVALID_INPUT);
				compatibilityVO.setStatusMessage(DMDConstants.STATUS_MESSAGE_INVALID_INPUT_DACC_COMPA);
			}else{
				compatibilityVO.setDacc(dacc);
				proceedToGetData = true;
			}
			if(proceedToGetData){
				LTEProcessor processor = new LTEProcessor();
				processor.getDaccSaccCompatibilityData(compatibilityVO);
			}
			
			String returnXML = LteXmlCreator.createDaccSaccCompatibilityXML(compatibilityVO);
			...
			if(statusCode.equals(DMDConstants.STATUS_CODE_ERROR)){//error
				statsLogBuf.append(compatibilityVO.getRequestInfo().getAppType())
							...
							.append(DMDConstants.DMD_PIPE)
							.append(DMDConstants.DACC_SACC_COMPATIBILITY_API)
							.append(DMDConstants.DMD_PIPE)
