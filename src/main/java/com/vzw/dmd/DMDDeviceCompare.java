...
import com.vzw.dmd.valueobject.DeviceCompareResponseVO;
import com.vzw.dmd.valueobject.DeviceCompareInfo;

public class DMDDeviceCompare extends HttpServlet implements ILteXmlCreator{
	private static Logger L =
		Logger.getLogger(DMDLogs.getLogName(DMDDeviceCompare.class));
	/**
	...
	*/
	public void defaultAction(HttpServletRequest req, HttpServletResponse res)
		...
		try{
			...
			statsLogBuf.append(DMDConstants.DMD_DEVICE_COMPARE);
			statsLogBuf.append(DMDConstants.DMD_PIPE);
			String deviceId = XmlUtils.getValue(document, "/dmd/requestBody/deviceId");
			L.debug("The device id  is :"+deviceId);
			String deviceSku = 	XmlUtils.getValue(document, "/dmd/requestBody/deviceSku");
			L.debug("The sim id  is :"+ deviceSku);
			
			if((deviceId == null ||deviceId.trim().equals("") ) || (deviceSku ==null ||deviceSku.trim().equals("")) ) {
				...
				returnXML.append("-1")
							...
							.append("Input device Id /device SKU  is empty.")
							.append(MESSAGE_END)
			...
			DeviceCompareInfo deviceCompareInfo = new DeviceCompareInfo();
			deviceCompareInfo.setDeviceId(deviceId);
			deviceCompareInfo.setDeviceSku(deviceSku);
			deviceCompareInfo.setDeviceType(DMDUtils.getDeviceIDType(deviceId));	
			 
			DeviceCompareResponseVO responseObj = DeviceLookupDAO.dmDeviceCompare( deviceCompareInfo);
			
			String returnXML = LteXmlCreator.createDeviceCheckXML(responseObj);
