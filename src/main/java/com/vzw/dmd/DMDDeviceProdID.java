...
public class DMDDeviceProdID extends HttpServlet {
	private static Logger L = Logger.getLogger(DMDLogs
			.getLogName(DMDDeviceType.class));
	...
	public void doPost1(HttpServletRequest req, HttpServletResponse res)
			throws ServletException, IOException {
		try {
	...
	public void postAction(HttpServletRequest request, HttpServletResponse res)
			throws ServletException, IOException {
		...
		//
		// Jun 2018 bqvt-89
		String prepayResStartDate = "";
		String postpayResStartDate = "";
		String wholesaleResStartDate = "";

		try {

			// xmlReqString =
			// "<DMD><APP_TYPE>MTAS</APP_TYPE><MACID>0013E09D560F</MACID><pESN></pESN></DMD>";
			...
			// inputSourceForXmlRequest = new InputSource(new
			// StringReader(xmlReqString));

			//
			/*
			 * DocumentBuilderFactory dbf =
			 * DocumentBuilderFactory.newInstance(); DocumentBuilder db =
			 * dbf.newDocumentBuilder(); L.debug("TEST1 "); Document document
			 * =null; L.debug("TEST21"+xmlReqString); document =
			 * db.parse(xmlReqString); L.debug("TEST2"); String req_type =
			 * XmlUtils.getValue(document, "DMD/APP_TYPE"); L.debug("TEST3");
			...
			if (xmlReqString != null) {
				...
				int clientNameStartIndex = xmlReqString
						.indexOf("<CLIENT_NAME>");
				...
				if (clientNameStartIndex != -1 && clientNameEndIndex != -1) {
					reqClientName = xmlReqString.substring(
							clientNameStartIndex + 13, clientNameEndIndex);
				...
				if (aplNameStartIndex != -1 && aplNameEndIndex != -1) {
					reqAplName = xmlReqString.substring(aplNameStartIndex + 10,
							aplNameEndIndex);
				...
				if (esnMeidStartIndex != -1 && esnMeidEndIndex != -1) {
					reqEsnMeid = xmlReqString.substring(esnMeidStartIndex + 10,
							esnMeidEndIndex);
				...
				if (deviceStartIndex != -1 && devicedEndIndex != -1) {
					String subPart = xmlReqString.substring(deviceStartIndex,
							devicedEndIndex);
			...
			if ((reqEsnMeid == null) && (reqEsnMeid == null)) {
				...
				createResponseXML(out, reqEsnMeid, resStatus, returnCode,
						returnMessage, prodName, mfgCode, mfgName, deviceType,
						prepayVO, launchVO, prepayResStartDate,
						postpayResStartDate, wholesaleResStartDate);
				...
				if (reqEsnMeid.trim().length() == 11) {
					...
					try {
						OracleDAOFactory oracleDAOFactory = OracleDAOFactory
								.getInstance();
						...
						EsnLookupRequestVO lookupResultVO = daoLookup
								.locateEsnMeid(lookupRequestVO);
						L.debug("Got Response for ESN Search Result Status: "
						...
						prepayResStartDate = chkNull(lookupResultVO
								.getPrepayRestrictStartDate());
						postpayResStartDate = chkNull(lookupResultVO
								.getPostpaidRestrictStartDate());
						wholesaleResStartDate = chkNull(lookupResultVO
								.getWholesaleRestrictStartDate());
						...
						deviceType = chkNull(daoLookup.getDeviceMask(prodName,
								mfgCode, effDate));
						OraclePibLockDAO daoPibLock = (OraclePibLockDAO) oracleDAOFactory
								.getPibLockDAO();
						prepayVO = daoPibLock.getRssPrepayData(esn);
						//
						// Get the VisionPROD ID and DeviceCategory FOR RIM
						// PREPAY
						DeviceLookupDAO lookup = new DeviceLookupDAO();
						launchVO = lookup
								.getGuiLaunchPackage_tst(lookupResultVO
										.getProdName());
						//
						L.debug("Got Response for ESN deviceType: "
								+ deviceType);
						returnCode = "00";
						returnMessage = "SUCCESS";
						resStatus = "Normal";
						if (prodName.trim().length() == 0
								&& mfgCode.trim().length() == 0) {
							returnCode = "02";
						...
						resStatus = "ERROR";
						resEsn = "Not Found";
						L.error(": getDeviceType(): failed during locating esn: "
								+ esn, ex);
						statsLogBuf.append(esn + "|");
						statsLogBuf.append("FALSE|");
					...
					createResponseXML(out, reqEsnMeid, resStatus, returnCode,
							returnMessage, prodName, mfgCode, mfgName,
							deviceType, prepayVO, launchVO, prepayResStartDate,
							postpayResStartDate, wholesaleResStartDate);

					/*
					 * DMDHubMacIdLookupRequestVO vo = new
					 * DMDHubMacIdLookupRequestVO(); vo.setMacid(esn);
					 * 
					 * OracleDAOFactory oracleDAOFactory =
					 * OracleDAOFactory.getInstance(); OracleEsnLookupDAO
					 * daoLookup =
					 * (OracleEsnLookupDAO)oracleDAOFactory.createEsnLookupDAO
					 * (); resEsn = daoLookup.getDMDHubPesn(vo);
					 * 
					 * 
					 * if (resEsn == null){ resStatus = "ERROR"; resEsn =
					 * "Not Found"; }else{ resStatus = "Normal"; }
					...
					String meid = DMDUtils
							.convertMEIDFromHexToDecimal(meid_hex);
					try {
						OracleDAOFactory oracleDAOFactory = OracleDAOFactory
								.getInstance();
						OracleEsnLookupDAO daoLookup = (OracleEsnLookupDAO) oracleDAOFactory
						...
						L.error(": getDeviceType(): failed during locating meid: "
								+ meid, ex);
					}
					createResponseXML(out, meid_hex, resStatus, returnCode,
							returnMessage, prodName, mfgCode, mfgName,
							deviceType, prepayVO, launchVO, prepayResStartDate,
							postpayResStartDate, wholesaleResStartDate);
					/*
					 ...
					 * daoLookup =
					 * (OracleEsnLookupDAO)oracleDAOFactory.createEsnLookupDAO
					 * (); resMeid = daoLookup.getDMDHubMacid(vo); if (resMeid
					 * == null){ L.debug("ERROR for pESN/Macid : "); resStatus =
					 * "ERROR"; resMeid = "Not Found"; }else{
					 * L.debug("Normal for pESN/Macid : "); resStatus =
					 * "Normal"; }
					...
					if (deviceID != null
							&& !deviceID.trim().equals(
									DMDConstants.EMPTY_STRING)) {
						deviceInfo.setDeviceIdType(DMDUtils
								.getDeviceIDType(deviceID));
						if (DMDConstants.DEVICE_TYPE_ESN.equals(deviceInfo
								.getDeviceIdType())) {
							deviceInfo.setDeviceId(DMDUtils
									.addLeadingZeros(deviceID));
						} else {
							deviceInfo.setDeviceId(deviceID);
						}
					}
					int deviceNotFound = DeviceLookupDAO
							.locateDevice(deviceInfo);
					L.debug("Device ID " + deviceID + "Device Found "
							+ deviceNotFound);
					// //IMEI
					if (deviceNotFound == DMDConstants.DMD_DEVICE_NOT_FOUND
							&& DMDConstants.DEVICE_TYPE_IMEI.equals(deviceInfo
									.getDeviceIdType())
							&& deviceInfo.getDeviceId().length() == 14) {
						deviceInfo
								.setDeviceIdType(DMDConstants.DEVICE_TYPE_MEID);
						deviceNotFound = DeviceLookupDAO
								.locateDevice(deviceInfo);
					}
					if (deviceNotFound == DMDConstants.DMD_DEVICE_NOT_FOUND) {
						String iccid = null;
						try {
							iccid = DeviceLookupDAO.lookupICCIDForEUIMID(
									deviceID.trim()).trim();
						} catch (Exception e) {
							iccid = null; // Ignore exception
						}
						...
						if (iccid == null || "".equals(iccid)) {
							...
							createResponseXML(out, deviceID, resStatus,
									returnCode, returnMessage, prodName,
									mfgCode, mfgName, deviceType, prepayVO,
									launchVO, prepayResStartDate,
									postpayResStartDate, wholesaleResStartDate);
						} else {
							returnCode = "00";
							returnMessage = "SUCCESS";
							resStatus = "Normal";
							createResponseXML(out, deviceID, resStatus,
									returnCode, returnMessage, iccid);
						}
					} else {
						...
						prepayResStartDate = chkNull(deviceInfo
								.getPrepayRestrictStartDate());
						postpayResStartDate = chkNull(deviceInfo
								.getPostpaidRestrictStartDate());
						wholesaleResStartDate = chkNull(deviceInfo
								.getWholesaleRestrictStartDate());
						DeviceLookupDAO lookup = new DeviceLookupDAO();
						launchVO = lookup.getGuiLaunchPackage_tst(deviceInfo
								.getProdName());
						/*
						 * OracleDAOFactory oracleDAOFactory =
						 * OracleDAOFactory.getInstance(); OracleEsnLookupDAO
						 * daoLookup = (OracleEsnLookupDAO) oracleDAOFactory
						 * .createEsnLookupDAO();
						 * if(deviceInfo.getDeviceIdType()
						 * .equals(DMDConstants.DEVICE_TYPE_ESN) ||
						 * deviceInfo.getDeviceIdType
						 * ().equals(DMDConstants.DEVICE_TYPE_MEID)) { String
						 * meid = "";
						 * deviceType=chkNull(daoLookup.getDeviceMask(prodName,
						 * mfgCode, effDate)); OraclePibLockDAO daoPibLock =
						 * (OraclePibLockDAO) oracleDAOFactory.getPibLockDAO();
						 * if(deviceInfo.getDeviceIdType().equals(DMDConstants.
						 * DEVICE_TYPE_MEID)) { meid=
						 * DMDUtils.convertMEIDFromHexToDecimal(meid_hex);
						 * prepayVO = daoPibLock.getRssPrepayData(deviceID);
						 * L.debug
						 * ("Got Response for ESN deviceType: "+deviceType); }
						 * else{ prepayVO =
						 * daoPibLock.getRssPrepayData(deviceID);
						 * L.debug("Got Response for ESN deviceType: "
						 * +deviceType); }
						 * 
						 * }
						...
						returnMessage = "SUCCESS";
						resStatus = "Normal";
						createResponseXML(out, deviceID, resStatus, returnCode,
								returnMessage, prodName, mfgCode, mfgName,
								deviceType, prepayVO, launchVO,
								prepayResStartDate, postpayResStartDate,
								wholesaleResStartDate);
					}
				} else {
					returnCode = "01";
					returnMessage = "INVALID ESN / MEID";
	...
	/*
	 * <DMD> <STATUS>Normal</STATUS> <ESN_MEID>05405349408<ESN_MEID>
	 * <PROD_NAME>LG-VX5200</PROD_NAME> <MFG_CODE>LGI</MFG_CODE>
	 * <DEVICE_TYPE>ODI</DEVICE_TYPE> </DMD>
	 */

	PrintWriter out, String resEsnMeid, String resStatus, String returnCode,
			String returnMessage, String resProdName, String resMfgCode,
			String resMfgName, String resDeviceType, RssPrepayVO prepayVO,
			LaunchPackageVO vo,
			// June 2018
			String prepayResStartDate, String postpayResStartDate,
			String wholesaleResStartDate) {
		try {
			L.debug("Got Response for ESN/MEID resStatus : " + resStatus);
			L.debug("Got Response for ESN/MEID resDeviceType : " + returnCode);
			L.debug("Got Response for ESN/MEID resDeviceType : "
					+ returnMessage);
			...
			Element child4 = xmlDoc.createElement("DEVICE_ID");
			 ...
			 * child9.appendChild(xmlDoc.createTextNode(prepayVO!=null?prepayVO.
			 * getPrepayEligibility():"")); root.appendChild(child9);
			 * 
			 * Element child10 = xmlDoc.createElement("ELIGIBILITY_DATE");
			 * child10
			 * .appendChild(xmlDoc.createTextNode(prepayVO!=null?prepayVO.
			 * getEligibilityDateMMDDYYYY():"")); root.appendChild(child10);
			 */
			Element child8 = xmlDoc.createElement("PROD_ID");
			child8.appendChild(xmlDoc.createTextNode(vo != null ? vo
					.getVisionProdId() : ""));
			root.appendChild(child8);

			Element child9 = xmlDoc.createElement("DEVICE_CATEGORY");
			child9.appendChild(xmlDoc.createTextNode(vo != null ? vo
					.getDeviceCategory() : ""));
			root.appendChild(child9);

			Element child10 = xmlDoc.createElement("prepayRestrictStartDate");
			child10.appendChild(xmlDoc.createTextNode(prepayResStartDate));
			root.appendChild(child10);

			Element child11 = xmlDoc.createElement("postpaidRestrictStartDate");
			child11.appendChild(xmlDoc.createTextNode(postpayResStartDate));
			root.appendChild(child11);

			Element child12 = xmlDoc
					.createElement("wholesaleRestrictStartDate");
			child12.appendChild(xmlDoc.createTextNode(wholesaleResStartDate));
	...
	private void createResponseXML(PrintWriter out, String deviceId,
			String resStatus, String returnCode, String returnMessage,
			String iccid) {
		try {
			L.debug("Got Response for ESN/MEID resStatus : " + resStatus);
			L.debug("Got Response for ESN/MEID resDeviceType : " + returnCode);
			L.debug("Got Response for ESN/MEID resDeviceType : "
					+ returnMessage);
			L.debug("Got Response for ESN/MEID deviceId : " + deviceId);
			L.debug("Got Response for ESN/MEID iccid : " + iccid);
			...
			Element child4 = xmlDoc.createElement("DEVICE_ID");
			child4.appendChild(xmlDoc.createTextNode(deviceId));
			root.appendChild(child4);

			Element child5 = xmlDoc.createElement("ICCID");
			child5.appendChild(xmlDoc.createTextNode(iccid));
			root.appendChild(child5);
