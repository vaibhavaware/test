...
public class DMDDeviceTypeSDM extends HttpServlet {
...
//							 //IMEI
							...
							if(deviceNotFound  == DMDConstants.DMD_DEVICE_NOT_FOUND){
								...
								if(deviceInfo.getDeviceIdType().equals(DMDConstants.DEVICE_TYPE_ESN) || deviceInfo.getDeviceIdType().equals(DMDConstants.DEVICE_TYPE_MEID))
								{
									...
									LTEProcessor proc = new LTEProcessor();
							        proc.getUniversalDeviceInfo(true, true, true, true, false, deviceInfo);
	...
	private void createResponseXML(
			...
			*/ 
			...
			try {
				...
				Element child6 = xmlDoc.createElement("MFG_NAME");
				child6.appendChild(xmlDoc.createTextNode(resMfgName));
				root.appendChild(child6);

				Element child7 = xmlDoc.createElement("DEVICE_TYPE");
				child7.appendChild(xmlDoc.createTextNode(resDeviceType));
